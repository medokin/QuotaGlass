# AI Status Overlay Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-08-25-ai-status-overlay-design.md`
...
- Store settings and the capped rolling log under `%APPDATA%\ai-status`.
...
- `AiStatus.slnx` - solution entry point.
- `Directory.Build.props` - nullable, implicit usings, deterministic build, warnings policy.
- `src/AiStatus/AiStatus.csproj` - Windows desktop executable and toast dependency.
- `src/AiStatus/Model/*` - immutable provider/report/alert data and pure severity policy.
- `src/AiStatus/Providers/*` - credential readers, HTTP mapping, and static provider registry.
- `src/AiStatus/Core/SettingsStore.cs` - defaults, atomic JSON persistence, and reload-on-change.
- `src/AiStatus/Core/RollingFileLog.cs` - body-free 1 MB log with one rotation.
- `src/AiStatus/Core/ThresholdWatcher.cs` - crossing detection and cycle-key deduplication.
- `src/AiStatus/Core/StatusPoller.cs` - parallel fetch, timeout, failure retention, and cadence.
- `src/AiStatus/Platform/*` - registry autostart, toast, hotkey, tray icon drawing, and activity state.
- `src/AiStatus/Ui/*` - shared provider card, popup, overlay, converters, and tray host.
- `tests/AiStatus.Tests/*` - provider, core, and pure UI-policy tests plus sanitized fixtures.
...
**Files:**
- Create: `AiStatus.slnx`
- Create: `Directory.Build.props`
- Create: `src/AiStatus/AiStatus.csproj`
- Create: `src/AiStatus/App.xaml`
- Create: `src/AiStatus/App.xaml.cs`
- Create: `src/AiStatus/Model/Enums.cs`
- Create: `src/AiStatus/Model/UsageWindow.cs`
- Create: `src/AiStatus/Model/ProviderSnapshot.cs`
- Create: `src/AiStatus/Model/StatusReport.cs`
- Create: `src/AiStatus/Model/StatusAlert.cs`
- Create: `src/AiStatus/Model/SeverityPolicy.cs`
- Create: `tests/AiStatus.Tests/AiStatus.Tests.csproj`
- Test: `tests/AiStatus.Tests/Model/SeverityPolicyTests.cs`
...
```powershell
dotnet new sln -n AiStatus --format slnx
dotnet new wpf -n AiStatus -o src/AiStatus -f net10.0
dotnet new xunit -n AiStatus.Tests -o tests/AiStatus.Tests -f net10.0
dotnet sln AiStatus.slnx add src/AiStatus/AiStatus.csproj tests/AiStatus.Tests/AiStatus.Tests.csproj
dotnet add tests/AiStatus.Tests/AiStatus.Tests.csproj reference src/AiStatus/AiStatus.csproj
dotnet add src/AiStatus/AiStatus.csproj package CommunityToolkit.WinUI.Notifications --version 7.1.2
```
...
namespace AiStatus.Tests.Model;
...
Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~SeverityPolicyTests`

Expected: compilation fails because `AiStatus.Model` does not exist.
...
```powershell
dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~SeverityPolicyTests
dotnet build AiStatus.slnx
```
...
```powershell
git add AiStatus.slnx Directory.Build.props src tests
...
**Files:**
- Create: `src/AiStatus/Providers/IStatusProvider.cs`
- Create: `src/AiStatus/Providers/ClaudeProvider.cs`
- Create: `tests/AiStatus.Tests/Support/StubHttpMessageHandler.cs`
- Create: `tests/AiStatus.Tests/Support/TemporaryDirectory.cs`
- Create: `tests/AiStatus.Tests/Providers/ClaudeProviderTests.cs`
- Create: `tests/AiStatus.Tests/Fixtures/FixtureSecurityTests.cs`
- Create: `tests/AiStatus.Tests/fixtures/claude-usage.json`
- Create: `tests/AiStatus.Tests/fixtures/claude-profile.json`
...
Write a test that enumerates `tests/AiStatus.Tests/fixtures/*`, rejects case-insensitive matches for `Bearer\s+[A-Za-z0-9._-]{20,}`, JWT shape `[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}`, and email shape `\b[^\s@]+@[^\s@]+\.[^\s@]+\b`.
...
Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter "FullyQualifiedName~ClaudeProviderTests|FullyQualifiedName~FixtureSecurityTests"`
...
```powershell
dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter "FullyQualifiedName~ClaudeProviderTests|FullyQualifiedName~FixtureSecurityTests"
dotnet test AiStatus.slnx
```
...
```powershell
git add src/AiStatus/Providers tests/AiStatus.Tests
...
**Files:**
- Create: `src/AiStatus/Providers/CodexProvider.cs`
- Create: `tests/AiStatus.Tests/Providers/CodexProviderTests.cs`
- Create: `tests/AiStatus.Tests/fixtures/codex-wham.json`
- Create: `tests/AiStatus.Tests/fixtures/codex-html-200.html`
...
Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~CodexProviderTests`
...
```powershell
dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter "FullyQualifiedName~CodexProviderTests|FullyQualifiedName~FixtureSecurityTests"
dotnet test AiStatus.slnx
```
...
```powershell
git add src/AiStatus/Providers/CodexProvider.cs tests/AiStatus.Tests
...
**Files:**
- Create: `src/AiStatus/Providers/OllamaProvider.cs`
- Create: `tests/AiStatus.Tests/Providers/OllamaProviderTests.cs`
- Create: `tests/AiStatus.Tests/fixtures/ollama-version.json`
- Create: `tests/AiStatus.Tests/fixtures/ollama-ps.json`
...
Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~OllamaProviderTests`
...
```powershell
dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~OllamaProviderTests
dotnet test AiStatus.slnx
```
...
```powershell
git add src/AiStatus/Providers/OllamaProvider.cs tests/AiStatus.Tests
...
**Files:**
- Create: `src/AiStatus/Core/AppPaths.cs`
- Create: `src/AiStatus/Core/AppSettings.cs`
- Create: `src/AiStatus/Core/SettingsStore.cs`
- Create: `src/AiStatus/Core/RollingFileLog.cs`
- Create: `tests/AiStatus.Tests/Core/SettingsStoreTests.cs`
- Create: `tests/AiStatus.Tests/Core/RollingFileLogTests.cs`
...
Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter "FullyQualifiedName~SettingsStoreTests|FullyQualifiedName~RollingFileLogTests"`
...
- `%APPDATA%\ai-status\settings.json`
- `%APPDATA%\ai-status\log.txt`
...
```powershell
dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter "FullyQualifiedName~SettingsStoreTests|FullyQualifiedName~RollingFileLogTests"
dotnet test AiStatus.slnx
git add src/AiStatus/Core tests/AiStatus.Tests/Core
...
**Files:**
- Create: `src/AiStatus/Core/ThresholdWatcher.cs`
- Create: `tests/AiStatus.Tests/Core/ThresholdWatcherTests.cs`
- Create: `tests/AiStatus.Tests/Support/SnapshotFactory.cs`
...
Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~ThresholdWatcherTests`
...
```powershell
dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~ThresholdWatcherTests
dotnet test AiStatus.slnx
git add src/AiStatus/Core/ThresholdWatcher.cs tests/AiStatus.Tests
...
**Files:**
- Create: `src/AiStatus/Core/StatusPoller.cs`
- Create: `tests/AiStatus.Tests/Core/StatusPollerTests.cs`
- Create: `tests/AiStatus.Tests/Support/FakeStatusProvider.cs`
...
Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~StatusPollerTests`
...
```powershell
dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~StatusPollerTests
dotnet test AiStatus.slnx
git add src/AiStatus/Core/StatusPoller.cs tests/AiStatus.Tests
...
**Files:**
- Create: `src/AiStatus/Platform/AutostartService.cs`
- Create: `src/AiStatus/Platform/ActivityStateMonitor.cs`
- Create: `src/AiStatus/Platform/GlobalHotkey.cs`
- Create: `src/AiStatus/Platform/ToastNotifier.cs`
- Create: `src/AiStatus/Platform/TrayIconRenderer.cs`
- Create: `src/AiStatus/Platform/TrayStatusPolicy.cs`
- Create: `tests/AiStatus.Tests/Platform/AutostartServiceTests.cs`
- Create: `tests/AiStatus.Tests/Platform/TrayStatusPolicyTests.cs`
...
Introduce internal `IRunKey` with `GetValue`, `SetValue`, and `DeleteValue`. Test `AutostartService.IsEnabled` reads the current value every time, `SetEnabled(true)` writes the quoted executable path under value name `AI Status`, and false deletes only that value. Production `RegistryRunKey` targets `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`.

- [ ] **Step 3: Run focused tests and verify failure**

Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter "FullyQualifiedName~TrayStatusPolicyTests|FullyQualifiedName~AutostartServiceTests"`
...
```powershell
dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter "FullyQualifiedName~TrayStatusPolicyTests|FullyQualifiedName~AutostartServiceTests"
dotnet build AiStatus.slnx
git add src/AiStatus/Platform tests/AiStatus.Tests/Platform
...
**Files:**
- Create: `src/AiStatus/Ui/Controls/ProviderCard.xaml`
- Create: `src/AiStatus/Ui/Controls/ProviderCard.xaml.cs`
- Create: `src/AiStatus/Ui/Converters/SeverityToBrushConverter.cs`
- Create: `src/AiStatus/Ui/Converters/PercentToGridLengthConverter.cs`
- Create: `src/AiStatus/Ui/Converters/ResetTimeConverter.cs`
- Create: `src/AiStatus/Ui/PopupWindow.xaml`
- Create: `src/AiStatus/Ui/PopupWindow.xaml.cs`
- Create: `src/AiStatus/Ui/OverlayWindow.xaml`
- Create: `src/AiStatus/Ui/OverlayWindow.xaml.cs`
- Create: `src/AiStatus/Ui/WindowPlacementService.cs`
- Create: `tests/AiStatus.Tests/Ui/DisplayConverterTests.cs`
...
Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~DisplayConverterTests`
...
```powershell
dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter FullyQualifiedName~DisplayConverterTests
dotnet build AiStatus.slnx
git add src/AiStatus/Ui tests/AiStatus.Tests/Ui
...
**Files:**
- Create: `src/AiStatus/Providers/ProviderRegistry.cs`
- Create: `src/AiStatus/Ui/TrayIconHost.cs`
- Modify: `src/AiStatus/App.xaml`
- Modify: `src/AiStatus/App.xaml.cs`
- Create: `tests/AiStatus.Tests/Ui/TrayTooltipTests.cs`
- Create: `tests/AiStatus.Tests/Providers/ProviderRegistryTests.cs`
...
Run: `dotnet test tests/AiStatus.Tests/AiStatus.Tests.csproj --filter "FullyQualifiedName~ProviderRegistryTests|FullyQualifiedName~TrayTooltipTests"`
...
```powershell
dotnet test AiStatus.slnx
dotnet build AiStatus.slnx -c Release
git add src tests
...
**Files:**
- Create: `src/AiStatus/Properties/PublishProfiles/win-x64.pubxml`
...
```powershell
dotnet test AiStatus.slnx -c Release
dotnet publish src/AiStatus/AiStatus.csproj -p:PublishProfile=win-x64
Get-ChildItem src/AiStatus/bin/Release/net10.0-windows10.0.19041.0/win-x64/publish
```

Expected: tests pass, publish succeeds, and the publish directory contains `AiStatus.exe` plus only required framework-dependent support files.
...
Expected: both hashes remain identical. Confirm `%APPDATA%\ai-status\settings.json` contains no access token, refresh token, account ID, email, or response body.

- [ ] **Step 6: Commit release assets**

```powershell
git add .gitignore src/AiStatus/Properties docs/manual-test-checklist.md
...
- [ ] Run `dotnet test AiStatus.slnx -c Release` and confirm zero failures.
- [ ] Run `dotnet build AiStatus.slnx -c Release` and confirm zero warnings.
