...
JSON at `%APPDATA%\ai-status\settings.json`, written atomically, reloaded on
...
- Access tokens live only in memory, are never logged, and are never written to
  settings or to disk.
- Logging: a single rolling text log at `%APPDATA%\ai-status\log.txt`, capped at
...
E:\ai-status\
  AiStatus.sln
  src\AiStatus\                    net10.0-windows, WPF
